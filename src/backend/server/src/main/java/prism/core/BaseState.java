...
/**
 * State in the base model. Used to encapsulate parser information together with information of the built model. Try not to use it.
 */
public class BaseState {
    ...
    public BaseState(long stateID, String stringValues, Model parent) throws PrismLangException {
        ...
        if (!stringValues.contains("=")){
            ...
            for (String id : ids) {
                ...
                for (Type t : valueTypes) {
                    try {
                        value = Model.castStringToType(assignment, t);
                        break;
            ...
            for (String id : ids) {
                ...
                for (Type t : valueTypes) {
                    try {
                        value = Model.castStringToType(assignment[1], t);
                        break;
                    } catch (PrismLangException e) {
                        value = null;
    ...
    public BaseState(long stateID, parser.State state, Model parent){
        this.stateID = stateID;
        this.state = state;
        this.parent = parent;
    }
    ...
    public Map<String, Object> getStateParameters(){
        ...
        for (int i = 0; i< state.varValues.length; i++){
            params.put(parent.getModulesFile().getVarName(i), state.varValues[i]);
        }
        return params;
    }
