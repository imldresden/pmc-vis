import { getPanes } from "../views/panes.js";
...
function makeParamSlider(opts) {
    const value = opts.subParam ? pane.cy.params[opts.param][opts.subParam] : pane.cy.params[opts.param];
    const $input = h('input', {
    ...
    const $label = h('p', { 'class': 'label label-default', for: 'slider-' + opts.param }, [t(opts.label)]);
...
function makeSchedulerPropDropdown() {
    const options = Object.keys(
        pane.cy.vars['details'].value[NAMES.results].props // only results from the 'details' 
...
function updatePropsValues() {
    ...
    Object.keys(original).forEach(d => {
        update[d] = { all: document.getElementById(`checkbox-${d}`).checked, props: {} };
        Object.keys(original[d].props).forEach(p => {
            update[d].props[p] = document.getElementById(`checkbox-${d}-${p}`).checked;
        });
    });
...
function makeDetailCheckboxes() {
    const $param = document.getElementById('props-checkboxes') || h('div', { 'class': 'param', 'id': 'props-checkboxes', 'style': 'display: block' });
    const $label = h('span', { 'id': 'props-checkboxes-label', 'class': 'label label-default' }, [t('Details to show')]);
    ...
    Object.keys(options).forEach(k => {
        ...
        $toggle.addEventListener('change', (e) => {
            ...
            pane.cy.vars['details'].fn(pane.cy, { update: updatePropsValues(), mode: false })
        });
...
function makeDetailPropsCheckboxes(options, propsName) {
    const toggles = [];
    const $param = h('div', { 'class': 'prop-checkboxes', 'id': `props-checkboxes-${propsName}`, 'style': 'display: block' });
    ...
    Object.keys(options).forEach(k => {
        ...
        const html = meta[k] && meta[k].identifier ?
            [meta[k].icon ?
...
function makePCPSettings() {
    ...
    countPrinter.innerHTML = `<pre id="count" style="height: 20px; font-size: 10px">${
        pane.cy.pcp ? 'Selected elements: ' +  pane.cy.pcp.getSelection().length : null
    }</pre>`;
    ...
    jsonPrinter.innerHTML = `<pre id="json" style="height: 500px; overflow-y:auto; font-size: 10px">${
        pane.cy.pcp ? JSON.stringify(pane.cy.pcp.getSelection(), undefined, 2) : null
    }</pre>`;
    ...
    $buttonExport.addEventListener('click', function () {
        if (pane.cy.pcp) {
            pane.cy.fns.export(pane.cy, pane.cy.pcp.getSelection().map(d => d.id));
        } else {
